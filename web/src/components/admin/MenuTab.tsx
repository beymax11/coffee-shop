...
export const MenuTab: React.FC<MenuTabProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* List Table */}
      <div className="rounded-2xl border border-white/[0.06] bg-[#0A0A0A]/40 backdrop-blur-sm overflow-hidden shadow-2xl">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[650px]">
            <thead>
              <tr className="border-b border-white/[0.06] bg-black/35 type-label text-[9px] tracking-wider text-zinc-400">
            ...
            >
              {filteredMenuItems.map((item) => (
                ...
                >
                  <td className="p-4 pl-6 flex items-center gap-4">
                    <div className="h-10 w-14 rounded-lg overflow-hidden border border-white/10 bg-zinc-900 shrink-0 relative shadow-inner">
                      <img
                        src={item.image}
                        alt={item.name}
                    ...
                    <div className="min-w-0">
                      <p className="type-body-sm font-semibold text-white truncate font-serif text-sm">{item.name}</p>
                      <p className="type-caption text-zinc-500 truncate text-[11px] max-w-xs mt-0.5">{item.description}</p>
                    </div>
                  </td>
                  <td className="p-4">
                    <span className="type-caption text-zinc-400 text-xs">{item.category}</span>
                  </td>
                  <td className="p-4 text-right">
                    <span className="type-price font-serif text-brand-gold text-[15px] font-bold">${item.price.toFixed(2)}</span>
                  </td>
                  <td className="p-4 text-center">
                    <div className="inline-flex items-center gap-1.5 bg-white/[0.02] border border-white/[0.05] rounded-full px-2.5 py-0.5">
                      <Star size={10} className="fill-brand-gold text-brand-gold" />
                      <span className="type-caption text-zinc-300 font-mono text-[10px] font-semibold">{item.rating.toFixed(1)}</span>
                    </div>
                  </td>
                  <td className="p-4 pr-6 text-center">
                    <div className="inline-flex gap-1.5">
                      <button
                        ...
                        className="p-2 rounded-lg hover:bg-brand-gold/10 text-zinc-400 hover:text-brand-gold transition-all duration-300 cursor-pointer"
                        title="Edit Item"
                      >
                        <Edit3 size={13} />
                      </button>
                      <button
                        ...
                        className="p-2 rounded-lg hover:bg-red-500/10 text-zinc-400 hover:text-red-400 transition-all duration-300 cursor-pointer"
                        title="Delete Item"
                      >
                        <Trash2 size={13} />
                      </button>
              ...
              {filteredMenuItems.length === 0 && (
                <motion.tr variants={rowVariants}>
                  <td colSpan={5} className="p-12 text-center text-zinc-500 italic type-body-sm bg-black/10">
